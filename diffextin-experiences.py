...
def main(argv=None):
    ...
    # maps_colorsWHWHa_SBHa_zones_sample_histogramsa(ALL, gals)
    ...
    WHa_SBHa_zones_sample_histograms(ALL, gals)
...
def WHa_SBHa_zones_sample_histograms(ALL, gals=None):
    if gals is None:
        _, ind = np.unique(ALL.califaID__z, return_index=True)
        gals = ALL.califaID__z[sorted(ind)]
    ...
    if (sel_gals__gz).any():
        ...
        if sel_WHa_DIG__gz.astype('int').sum() > 0:
        ...
        if sel_WHa_COMP__gz.astype('int').sum() > 0:
        ...
        if sel_WHa_HII__gz.astype('int').sum() > 0:
        ...
        axS.grid()
        f.savefig('dig-sample-WHa_SBHa-classifWHa_zones.png')
        plt.close(f)

        x = np.ma.log10(W6563__gz)
        y = np.ma.log10(SB6563__gz)
        f = plt.figure(figsize=(8, 8))
        x_ds = [x[sel_Zhang_DIG__gz].compressed(), x[sel_Zhang_COMP__gz].compressed(), x[sel_Zhang_HII__gz].compressed()]
        y_ds = [y[sel_Zhang_DIG__gz].compressed(), y[sel_Zhang_COMP__gz].compressed(), y[sel_Zhang_HII__gz].compressed()]
        axS, axH1, axH2 = plot_scatter_histo(x_ds, y_ds, logWHa_range, logSBHa_range, 30, 30,
                                             figure=f, c=['r', 'g', 'b'], scatter=False, s=1,
                                             ylabel=r'$\log\ \Sigma_{H\alpha}$ [L${}_\odot/$kpc${}^2$]',
                                             xlabel=r'$\log$ W${}_{H\alpha}$ [$\AA$]')
        ...
        if sel_Zhang_DIG__gz.astype('int').sum() > 0:
        ...
        if sel_Zhang_COMP__gz.astype('int').sum() > 0:
        ...
        if sel_Zhang_HII__gz.astype('int').sum() > 0:
        ...
        axS.grid()
        f.savefig('dig-sample-WHa_SBHa-classifZhang_zones.png')
        plt.close(f)
